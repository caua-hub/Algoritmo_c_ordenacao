...
//funcao que conta a quantidade de valores que há no vetor alocado
...
void counting_sort(int *vetor, int tamanho, int aleatorio);
                ...
                switch(op3){
                    ...
                    case 'e'://quick sort
                        ...
                        for(int i=0;i<VEZES;i++){
                            ...
                            vetor_copia(v_auxiliar,v_alocar,tam_alocado);
...
void gerar_relatorio(char *arquivo){
    ...
    if(fez_linear == 1){ //se a busca linear foi feita
    ...
    }else if(fez_binaria == 1){ //se a busca binaria foi feita
        ...
        fprintf(relatorio,"Nenhuma busca feita.\n"); //nenhum tipo de busca foi feito
    }
...
void insertion_sort(int *vetor, int tamanho){
    ...
    for(int i= 1; i < tamanho; i++){
        temp = *(vetor + i);   //'temp' guarda o valor atual(chave)
        j = i - 1;             // posição anterior

        //move os valores maiores que 'temp' uma posição pra frente no vetor
        while((j >= 0) && (temp < *(vetor + j))){
            *(vetor + (j + 1)) = *(vetor + j); //move o elemento para a direita
            j--; 
        }
        *(vetor + (j + 1)) = temp; //insere o valor(temp) na posição correta
    }
}
    ...
    for(int i = 0; i < tamanho-1; i++){
        ...
        for(int j = i+1; j < tamanho; j++){
            if(*(vetor + j) < *(vetor + menor))
                menor = j; //atualiza a posição do menor encontrado
        }

        if(i != menor){ //se encontrou um elemento menor troca
            temp = *(vetor + i);
            *(vetor + i) = *(vetor + menor);
            *(vetor + menor) = temp;
        }
...
void counting_sort(int *vetor, int tamanho, int aleatorio){
    ...
    for(i=tamanho-1;i>=0;i--){
        num = (*(vetor + i) / aleatorio) % 10;
        array_saida[contador[num] - 1] = *(vetor + i);
        contador[num]--;
    }

    for(i=0;i<tamanho;i++){
        *(vetor + i) = array_saida[i];
    }
...
void radix_sort(int *vetor, int tamanho){
    ...
    for(aleat=1;maior_num/aleat > 0;aleat*=10){
        counting_sort(vetor,tamanho,aleat);
    }
